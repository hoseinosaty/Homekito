...
namespace Barayand.DAL.Repositories
{
    public class RelationProductRepository: GenericRepository<RelationProductRepository>,IPRRepository
    {
        ...
        public async Task<ResponseStructure> UpdateRelation(List<RelatedProductModel> data,int prodid)
        {
            try
            {
                var rel = this._context.RelatedProduct.ToList();
                this._context.RelatedProduct.RemoveRange(rel.Where(x => x.X_MainProdId == prodid).ToList());
